...
@interface WZCCalendarLogic ()
{
    NSDate *today;//今天的日期
    NSDate *before;//之后的日期
    NSDate *select;//选择的日期
    WZCCalendarDayModel *selectcalendarDay;
}

@end


@implementation WZCCalendarLogic
...
- (NSMutableArray *)calculateDaysInPreviousMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array
{
    ...
    for (int i = daysCount - partialDaysCount + 1; i < daysCount + 1; ++i) {
        ...
        WZCCalendarDayModel *calendarDay = [WZCCalendarDayModel calendarDayWithYear:components.year month:components.month day:i];
...
- (void)changStyle:(WZCCalendarDayModel *)calendarDay
{
...
-(void)LunarForSolarYear:(WZCCalendarDayModel *)calendarDay{
